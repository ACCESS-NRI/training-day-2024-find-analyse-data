...
## Step 0: Pre-workshop
- *0.0* Hopefully you followed the prerequisite guidelines previously provided and you already have an NCI account and have joined the following NCI projects: `nf33`, `xp65`, `dk92`, `fs38`, `rr3`, `al33`, `p73`, `ct11`, `hh5` and `oi10`. If this is not the case, please talk to a helper.

- *0.1* Log in to Gadi via the command line using your **NCI username and password**.
  
   ```bash
  ssh <your_nci_username>@gadi.nci.org.au
  ```

- *0.2* Clone the [workshop-training-2023](https://github.com/ACCESS-NRI/workshop-training-2023/tree/main) Github repo to your user directory within `/scratch/nf33` on Gadi. This repo contains the training material needed for the afternoon. Depending on whether or not you've used the `nf33` project before, your user directory may or may not already exist.
